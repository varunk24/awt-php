...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="3mgdoctor.php" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-users fa-fw"></i> Manage Doctors </a>
...
<header class="w3-container" style="padding-top:18px">
    <h5><b><i class="fa fa-dashboard"></i>Manage Doctors</b></h5>
  </header>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <?php $i=1;?>
      <?php foreach($ddisp as $dcomp): ?>
      <th scope="row"><?php echo $i; ?></th>
      <td><?php echo $dcomp['dname']; ?></td>
      <td><?php echo $dcomp['hname']; ?></td>
      <td><?php echo $dcomp['created_at']; ?></td>
      <td><div>
      <form action=<?php echo $_SERVER['PHP_SELF']; ?> method="POST"> 
      <input type="hidden" name="did" value="<?php echo $dcomp['id']; ?>">
      <?php if($dcomp['status']!=2){ ?>
          <button class="btn btn-primary " name="ddisable" >Disable</button>
        <?php }?>
        <?php if($dcomp['status']==2){ ?>
          <button class="btn btn-success " name="denable" value="more">Enable</button>
        <?php }?>
      <button class="btn btn-danger" name="ddelete" value="disable">Delete</button></form></div>
      </td>
      <?php  $i++; ?>
    </tr>
      <?php endforeach; ?>
