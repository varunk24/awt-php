...
require('config/cviewappointment.php');
...
<header>
  ...
  <?php foreach($list as $apt): ?>
    <div id="items"  style="margin:3%;min-width:70%;border:solid 2px ;border-radius:10px;padding:2em;min-height:24em;">
        ...
        <?php if($apt['status']<3){ ?>
        <form method="POST">
        <button class="btn btn-danger float-right" name="delete" value="delete" >Cancel</button>
        <input type="hidden" name="aptid" value="<?php echo $apt['id']; ?>">
        </form>
        <?php } ?>
        </div>
        <div class="float-left">
        <h3>Doctor Name</h3>
        <small><?php echo $apt['dname']; ?></small>
        <h3>Hospital Name</h3>
        <small><?php echo $apt['hname']; ?></small>
        <h3>Time</h3>
        <small><?php echo $apt['time']; ?>am/pm</small>
        <h3>Date</h3>
